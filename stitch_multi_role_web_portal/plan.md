...
- sessions: domain_id, session_id, visitor_id, duration_seconds, page_count, country, device, entry_url, exit_url, utm_source, utm_medium, utm_campaign, started_at
...
- /dashboard/visitors — paginated visitor table with filters; click row → session detail drawer with timeline of events, scroll map, time-on-page, path
...
5. PostgreSQL migrations: all tables (users with timezone + email_verified_at + locale + appearance + onboarding json + totp columns, totp_backup_codes, plans with data_retention_days + name_ar + description_ar, payment_methods with name_ar, theme_settings seeded with 12 default keys, domain_exclusions, pipelines, pipeline_steps, ai_reports, ai_suggestions, audience_segments, visitor_identities, webhooks, webhook_deliveries, shared_reports, saved_views, export_jobs, impersonation_logs, audit_logs, alert_rules, notifications with email_sent_at + action_url + channel, notification_preferences, visitor_optouts, data_deletion_requests)
6. ClickHouse migrations: events (with utm_* columns), sessions (with utm_*), pipeline_events, ux_events, custom_events — all tables MergeTree engine, partitioned by toYYYYMM(created_at)
...
## Open Questions
1. **IP Geolocation:** MaxMind GeoLite2 (free, self-hosted, no rate limit) confirmed — requires registering for a license key; add to deployment docs.
