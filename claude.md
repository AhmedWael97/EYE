...
| Jun 2026 | Email verification blocked dashboard access (forced redirect to verify-email) | Verification is now **non-blocking**: `(app)/layout.tsx` no longer redirects unverified users; instead `components/VerifyEmailBanner.tsx` shows a dismissable top bar with a one-click resend (`authApi.resendVerification`). Whether the email is actually sent still depends on `app.email_verification_enabled`. The verify-email page also has a "Verify later" → dashboard link. |
| Jun 2026 | Prices only in USD; Paymob charged wrong amount (USD number as EGP) | **Geo currency**: base plan prices stay USD. Public `GET /geo/currency` (`GeoCurrencyController` via `GeoIpService`) returns EGP+rate for Egyptian IPs, else USD. Frontend `lib/useCurrency.ts` hook formats prices (billing + marketing pricing pages). Rate in `config/services.php → currency.egp_rate` (env `CURRENCY_EGP_RATE`, default 60). **Paymob always charges EGP**: `initiate()` converts `price_usd × rate` → `amount_cents`, and stores the Payment in EGP so the webhook's amount reconciliation matches. Non-Egypt users see `$` but are charged the EGP equivalent (disclosed in the Paymob note). |
